			...
			<view class="exam-item-inner" v-if='dataList.length > 0'>
				...
				<view v-show="currentItem.questionType == 2">
					<view class="exam-item-option">
						<textarea @blur="bindTextAreaBlur" @input='bindTextinput' placeholder='请输入您的答案'
							:value='getTxtValue(currentIndex)' />
					</view>
				</view>
				...
				<view class="exam-button-row">
					<view>
						...
						<button v-show="currentIndex != 0" size='mini' @tap="lastQuestion"
							style="margin-right: 20rpx;">{{lastText}}</button>
						<button v-show="currentIndex < dataList.length-1" size='mini'
							@tap="nextQuestion">{{nextText}}</button>
